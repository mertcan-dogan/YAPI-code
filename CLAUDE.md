...
- **`backend/`** — Python FastAPI + SQLAlchemy 2.0 + Alembic; PostgreSQL (Supabase, Row-Level Security).
  ...
  - `migrations/versions/` — Alembic DB migrations. **Current head: `0041_alembic_version_app_read`. Next id: 0042.**
...
3. **Only ONE migration in flight at a time.** If your task needs a DB schema change, the new migration is numbered after the current head (today 0039) and must be the ONLY new one. If you're told another agent owns migrations for now, do **not** add one.
...
5. **No migration ⇒ confirm `alembic heads` is unchanged (0038).** A new migration ⇒ it must be the single new head; validate it applies + downgrades on a local Postgres before merge (never test against prod).

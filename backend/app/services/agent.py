...
_ACTION_GUIDANCE = (
    ...
    "(x,y,w,h) almalı. AYNI metrik+kırılım+filtre tablosunu iki kez EKLEME; projenin "
    "doldurmadığı bir boyut için kırılım widget'ı ekleme; başlıkta '%' varsa metrik de "
    "yüzde olsun (₺ metriği '%'-başlıkla eşleştirme). TASLAĞI ekle ve kullanıcıya "
    "düzenleyebileceğini veya OLUŞTUR'a basabileceğini söyle.\n"
    ...
    "yıllardaysa aylık nakit/maliyet SIFIR çıkar. AYNI tabloyu iki kez ekleme, '%'-başlığı "
    "yalnızca yüzde metrikte kullan. TASLAĞI ekle; kullanıcı 'Beceri olarak "
    "kaydet'e basınca kaydeder.\n"
    "- TASLAK ELEŞTİRİSİ (CR-056): taslak eklenince yinelenen/etiketi uyumsuz widget'lar "
    "otomatik işaretlenir ve kullanıcıya seçeneklerle sorulur. Sen planı KENDİN değiştirme; "
    "araç sonucunda fark edilenleri KISACA söyle ve ne yapmak istediğini SOR (kullanıcının "
    "tıklaması planı kırpar).\n"

...
def upgrade() -> None:
    ...
    for table in SOFT_DELETE_TABLES:
        op.execute(
            f"""
            CREATE POLICY {table}_hide_deleted ON {table}
            FOR SELECT
    ...
    op.execute(
        """
        CREATE POLICY audit_log_append_only_no_update ON audit_log
        FOR UPDATE USING (false);
        """
    )
    op.execute(
        """
        CREATE POLICY audit_log_append_only_no_delete ON audit_log
